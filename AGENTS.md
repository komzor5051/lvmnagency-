...
**Blog content is Codex/Codex focused** (pivoted 2026-07-08/09, see
`docs/superpowers/specs/2026-07-08-Codex-content-pivot-design.md`): 4 pillars —
Codex.ai for non-technical users, Codex for developers, business automation
via Codex, comparisons/news. Old AI-automation/n8n-generic topics were retired.
...
System cron (server crontab, CRON_TZ=UTC — replaces the old Vercel Cron)
  ...
  │     └── Exa trends + docs.Codex.com/anthropic.com scrape → Gemini → save topics
  │         (4 pillars: Codex.ai, Codex, business automation via Codex, comparisons/news)
  │
  └── /api/cron/generate     (daily, 05:00 UTC = 08:00 MSK — BROKEN, see note above)
        ...
        │         Codex-pillar articles are grounded in Влад's real skills/MCP
        │         setup via lib/Codex-setup-context.ts, not generic web advice)
...
- `lib/Codex-setup-context.ts` — Влад's real Codex setup (skills, MCP
  servers, subagent/workflow patterns), client-free; writer.ts grounds
  "Codex для разработчиков" pillar articles in this instead of generic
...
validation was dropped when the blog pivoted to Codex content) — remove it
